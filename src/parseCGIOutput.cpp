...
void	CGI::_lexInput(std::string const &str)
{
	...
	for (std::string::const_iterator it = start; it != str.end(); it++)
	{
		if (*it == '"' && inQuotes)
		{
			inQuotes = false;
			word += *it;
			this->_CGItokenList.push_back(token(word, QUOTED));
			word.clear();
		}
		else if (*it == '"' && !inQuotes)
		{
		...
		else if (isspace(*it) && !inQuotes)
		{
			if (word != "")
			{
				this->_CGItokenList.push_back(token(word, WORD));
				word.clear();
			}
			if (*it == '\n')
			{
			...
			word += *it;
			this->_CGItokenList.push_back(token(word, SPACE));
			word.clear();
		}
...
int	CGI::lexCGIOutput(std::string &data)
{
	...
	std::list<t_reqToken>::reverse_iterator	rit = this->_CGItokenList.rbegin();
	while (rit->type != CRLF && rit != this->_CGItokenList.rend())
		rit++;
